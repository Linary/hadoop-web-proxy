...
// 访问URL：http://localhost:8080/hadoop-web-proxy/v1/work/20151104023705_10.58.188.53.log

public class DownloadServer extends HttpServlet {

	// 常量定义
	private static final long serialVersionUID = 1L;
	private static final String SERVLET_VERSION = "v1";
	...
	/** 
	 * 准备一些文件信息相关的临时变量，后面需要用到，包括：
	 * 文件路径、文件名、文件大小、上一次修改时间、过期时间、Etag
	 */
	protected class Aux {
	...
	public void init() throws ServletException {
		// HDFS的路径
		HDFSAddress = this.getInitParameter("HDFSAddress");
		if (HDFSAddress == null) {
        	// 这里应该要记录日志比较好
			System.out.println("需要指定下载文件的根目录");
			// TODO:
			System.exit(0);
        }
		// 根据本地配置文件（core-site.xml、hdfs-site.xml）初始化配置对象
		// TODO:conf.addResource()
	 ...
	 * @param req 客户端发送给服务器的HTTP请求
	 * @param res 服务器发送给客户端的HTTP响应
	 * @throws IOException 访问资源的地址错误
	 */
	private Aux initializeAuxObject(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		// 寻找到第一个"v1"出现的位置
		int fileIndex = requestURI.indexOf(SERVLET_VERSION) + SERVLET_VERSION.length() + 1;
		
		// 后面多次需要用到的 Path 对象
		aux.hdfsPath = new Path(HDFSAddress + requestURI.substring(fileIndex));
		// 得到 hdfs 的文件系统，这里可以不是同一个HDFS
		aux.fileSystem = FileSystem.get(URI.create(aux.hdfsPath.toString()), conf);
		// 检查该路径的文件是否存在、且指定路径是文件而不是目录
		if (!aux.fileSystem.exists(aux.hdfsPath) || !aux.fileSystem.isFile(aux.hdfsPath)) {
			res.sendError(HttpServletResponse.SC_NOT_FOUND);
			return null;
		}
		
		// 得到文件状态信息
		FileStatus fileStatus = aux.fileSystem.getFileStatus(aux.hdfsPath);
		// 后续几个方法需要用到的成员变量
		String filePath = fileStatus.getPath().toString();
		aux.fileName = filePath.substring(filePath.lastIndexOf("/") + 1);
...
//			res.setHeader("Content-Range", "bytes */" + aux.totalLength);
	 ...
	 * @throws IOException
	 */
	private void sendRequestedFile(HttpServletRequest req, HttpServletResponse res, Aux aux) throws IllegalArgumentException, IOException {
		// 为什么需要 reset()
		res.reset();
		// config
		res.setBufferSize(DEFAULT_BUFFER_SIZE);
		res.setHeader("content-disposition", "attachment; filename=\"" + aux.fileName + "\"");
		
		// response 压缩设置
		res.setHeader("Content-Encoding", "gzip");
		
		// hdfs 文件的输入流，用于读取hdfs文件
		FSDataInputStream input = aux.fileSystem.open(aux.hdfsPath);
		...
		try {
			if (aux.ranges.size() == 1) {
				...
				// 每一 multipart 段中都应包含 Content-Range域用以指示本段的内容范围
				// todo
				res.setContentType("multipart/byteranges; boundary=MULTIPART_BYTERANGES");
				res.setStatus(HttpServletResponse.SC_PARTIAL_CONTENT);
				// 循环拷贝每一段
	 ...
	 * 提取指定范围内的子字符串的长整型值
	 * @param str 源字符串
	 * @param beginIndex 范围起始索引
	 * @param endIndex 范围终止索引
